...
## 4.splitted time stamp column into 'year' and 'the rest'
# 4.1
...
# 5. assigned all the columns, I wanted to drop to an object called 'drop_year'(first identify what columns are removed because they're not considered candy - 'not_candy_year', and the ones that are irrelevant to the analysis - 'irrelevant_col_year')
...
# 8.2 shorten column names to 20 characters
...
#  replace countries that have a number as a value to 'US' and then replacing all potential entries that suggest UK as a country with 'UK' and 'Canada'.
